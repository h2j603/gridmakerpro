...
function renderCanvas() {
  ...
  viewport.innerHTML = getSortedLayers().map(layer => {
    ...
    const modulesHTML = orderedModules.map((moduleData, i) => {
      ...
      // aspect-ratio 수정: grid-row는 항상 span 유지
      ...
      const hasAspectClass = moduleData.aspectRatio ? 'has-aspect-ratio' : '';
      
      const backgroundStyle = (moduleType === 'box') ? `background: ${bgColor};` : '';

      return `
      <div class="module ${selectedClass} ${groupedClass} ${showWarning ? 'warning' : ''} ${hasAspectClass}" 
...
`;
    layer.modules.forEach(m => {
      ...
      // 코드 생성에서도 grid-row는 항상 span 유지
      const aspect = m.aspectRatio ? `\n  aspect-ratio: ${m.aspectRatio};` : '';
      const row = `span ${m.row}`;  // 항상 span 값
